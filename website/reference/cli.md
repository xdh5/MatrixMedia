...
| `--publish-at`            | 一次性定时发布，格式 `YYYY-MM-DD HH:mm:ss`；抖音、快手、视频号、百家号、头条使用平台官方定时，其他平台使用矩媒本地调度 |
...
6. **官方预约发布**：`--publish-at "YYYY-MM-DD HH:mm:ss"` 只支持抖音、快手、视频号、百家号和头条。矩媒会立即上传，并在平台后台确认官方预约成功后返回；其他平台直接报错，不再创建应用内定时任务。
